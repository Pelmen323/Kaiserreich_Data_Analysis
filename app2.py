...
def decompress_data_dict(input_str: str, flag=None) -> dict:
    try:
        if flag == "WT":
            return {i[:i.index(':')]:float(i[i.index(':')+1:]) for i in input_str.split(',')}              # "36.02:0.019,36.03:0.087,36.04:0.085" -> {'36.02': '0.019', '36.03': '0.087', '36.04': '0.085'}
        return {i[:i.index(':')]:i[i.index(':')+2:-1].split(",") for i in input_str.split('],')}           # "36.03:[AUS;26;73;118,JAP;32;65;83],36.06:[AUS;26;73;130,JAP;37;62;91]" -> {'36.03': ['AUS;26;73;118', 'JAP;32;65;8'], '36.06': ['AUS;26;73;130', 'JAP;37;62;9']}
    except:
        print(input_str)
...
@app.callback(
    ...
    Output("world-tension-tt", "children"),
    Input("world-tension-data-source", "value"),
    Input("world-tension-filter-by-color", "value"))
def create_world_tension_graph(input_file, filter_color):
    csv_df = read_csv_file(input_file)

    if "WT Data" in csv_df.columns: 
        fig = px.line(data_frame=pd.DataFrame(columns=["Time", "World Tension"]), x="Time", y="World Tension")          # 1. Create empty graph with no traces
        counter_red, counter_orange, counter_yellow, counter_green = 0, 0, 0, 0
        series = csv_df["WT Data"]                                                                                      # 2. Create series (one column with WT Data answers)

        for i in series:                                                                                                # 3. Parse through series rows and create scatter for each row (for each reported log)
            series_row = decompress_data_dict(i, flag="WT")                                                             # 4. Convert input line into dictionary. Keys represent Date and Values - WT values from 0 to 1

            if "40.12" in series_row.keys() and series_row["40.12"] < 0.75:
                color = "Red"
                counter_red += 1
            elif "40.07" in series_row.keys() and series_row["40.07"] < 0.75:
                color = "Orange"
                counter_orange += 1
            elif "38.12" in series_row.keys() and series_row["38.12"] > 0.75:
                color = "Yellow"
                counter_yellow += 1
            else:
                color = "Green"
                counter_green += 1
            if filter_color == "All" or filter_color == color:
                fig.add_scatter(x=[i for i in series_row.keys()], y=[i for i in series_row.values()], marker=dict(size=3, color=color)) # 5. Add scatter - x represents date axis, y - WT values

        world_tension_tt = f'''
        {len(series)} graphs total:
        - {counter_red} graphs with insufficient WT (< 75% WT by 12.1940);
        - {counter_orange} graphs with late Weltkriegs (< 75% WT by mid 1940);
        - {counter_yellow} graphs with early Weltkriegs (> 75% WT by 12.1938);
        - {counter_green} graphs with balanced WT (75% WT between 01.1939 and 06.1940).'''

        fig.update_layout(plot_bgcolor=colors["background"], paper_bgcolor=colors["background"], font_color=colors["text"], legend_title="Country")
        fig.update_traces(line=dict(width=1))
        fig.update_xaxes(showgrid=False, gridwidth=line_widths["grid_xaxis"], gridcolor=colors["grid"])
        fig.update_yaxes(showgrid=False, gridwidth=line_widths["grid_yaxis"], gridcolor=colors["grid"])

        return fig, world_tension_tt
    else:
...
app.layout = html.Div(
    ...
    children=[
        ...
        html.Span(children="World Tension", id="wt-section", className="section-header"),
        dcc.Dropdown(id="world-tension-data-source", options=get_dropdown_options(), value="January 2023.csv", clearable=False),
        dcc.Dropdown(id="world-tension-filter-by-color", options=["Red", "Orange", "Yellow", "Green", "All"], value="All", clearable=False),
        dcc.Graph(id="world-tension-graph"),
        dcc.Markdown(id="world-tension-tt", className="md-tooltip"),
        html.Br(),

        # html.Span(children="The Second Weltkrieg", id="2wk-section", className="section-header"),
        ...
        # html.Span(children="Argentinean-Chilean War", id="argentina-section", className="section-header"),
        ...
        # html.Span(children="Spanish Civil War", id="scw-section", className="section-header"),
        ...
        # html.Span(children="American Civil War", id="acw-section", className="section-header"),
        ...
        # html.Br(),
        # html.Span(children="Industry Map", id="map-section", className="section-header"),
